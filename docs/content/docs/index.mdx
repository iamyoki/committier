...
<Card icon={<AppWindow className="text-purple-300" />} title='UX / DX Driven Design'>

Starting from a zero-configuration setup, we provide you with a beautiful and highly consistent commit message formatting.

</Card>

<Card icon={<CpuIcon className="text-blue-300" />} title='Adhere conventioanl commits style'>
...
<Card icon={<Ruler className="text-green-300" />} title='Compatible with commitlint'>

Perfect and without conflict. Taking commit message quality to the next level.

</Card>

<Card icon={<Workflow className="text-yellow-300" />} title='Automatically format'>
...
<Card icon={<Terminal className="text-slate-100" />} title='Commit CLI tool'>
